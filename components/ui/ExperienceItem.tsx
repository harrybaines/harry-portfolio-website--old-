...
export default function ExperienceItem({ job, minimal }) {
    ...
    return (
      <>
        ...
        <div className="col-span-2 flex items-center justify-end">
          <p className="text-gray-400 text-right">{job.date}</p>
        </div>
      </>
    );
  }
  return (
    <>
      <div className="col-span-1 text-gray-500">{job.date}</div>
        ...
        <div className="flex flex-wrap mt-3 gap-y-3">
          {job.skills.map((skill, index) => (
            <div key={skill} className="flex items-center">
              <span className="text-gray-400 bg-gray-800 rounded-xl px-2 text-sm">
                {skill}
              </span>
